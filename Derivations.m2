...
Derivation.synonym = "ring map"
matrix Derivation := opts -> f -> f.matrix
source Derivation := f -> f.source
target Derivation := f -> f.target
raw Derivation := f -> f.RawDerivation

expression Derivation := f -> (expression map) (expression (target f, source f, first entries matrix f))
toString Derivation := f -> toString expression f
net Derivation := f -> net expression f
texMath Derivation := x -> texMath expression x

describe Derivation := f -> Describe expression f
toExternalString Derivation := f -> toString describe f
-- should do something about the degree map here

degmap0 := n -> ( d := toList ( n : 0 ); e -> d )

derivation(RingFamily,Thing,Thing) := Derivation => opts -> (R,S,m) -> derivation(default R,S,m,opts)
derivation(Thing,RingFamily,Thing) := Derivation => opts -> (R,S,m) -> derivation(R,default S,m,opts)

workable = f -> try (f(); true) else false

derivation(Ring,Ring,Matrix) := Derivation => opts -> (R,S,m) -> (
     ...
     new Derivation from {
	  ...
	  symbol source => S,
	  symbol matrix => mE,
	  symbol RawDerivation => rawDerivation m.RawMatrix,
...
map(Ring,Matrix) := Derivation => opts -> (S,m) -> map(ring m,S,m)

map(Ring,Ring) := Derivation => opts -> (S,R) -> map(S,R,{},opts)

Ring#id = (R) -> map(R,R,vars R)

Derivation#{Standard,AfterPrint} = Derivation#{Standard,AfterNoPrint} = f -> (
     << endl;				  -- double space
     << concatenate(interpreterDepth:"o") << lineNumber << " : " << class f;
     << " " << target f << " <--- " << source f << endl;
     )

Derivation RingElement := RingElement => fff := (p,m) -> (
     R := source p;
     S := target p;
     if R =!= ring m then (
	  m = try promote(m,R) else error "ring element not in source of ring map, and not promotable to it";
	  );
     promote(rawDerivationEval(raw p, raw m),S))

Derivation Number := (p,m) -> fff(p, promote(m,source p))

Derivation Matrix := Matrix => (p,m) -> (
     R := source p;
     S := target p;
     if R =!= ring m then (
     ...
     F := p target m;
     E := p source m;
     map(F,E,map(S,rawDerivationEval(raw p, raw cover F, raw m)), Degree => p.cache.DegreeMap degree m))

Derivation MutableMatrix := MutableMatrix => (p,m) -> (
     R := source p;
     S := target p;
     if R =!= ring m 
     then error "expected source of ring map to be the same as ring of matrix";
     map(S,rawDerivationEval(raw p, raw m)))

Derivation Vector := Vector => (p,m) -> (
     f := p new Matrix from m;
     new target f from f)

kernel = method(Options => { SubringLimit => infinity })
kernel Derivation := Ideal => opts -> (cacheValue (symbol kernel => opts)) (
     (f) -> (
	  R := source f;
	  n2 := numgens R;
	  F := target f;
	  ...
	  then (
	       ...
	       if R' === R and F' === F then error "kernel Derivation: not implemented yet";
	       p^-1 kernel (r * f * p^-1))))

coimage Derivation := QuotientRing => f -> f.source / kernel f

Derivation * Derivation := Derivation => (g,f) -> (
     ...
     new Derivation from {
	  ...
	  symbol matrix => m,
	  symbol RawDerivation => rawDerivation raw m,
...
map(Ring,Ring,List) := Derivation => opts -> (S,R,m) -> (
...
Derivation Ideal := Ideal => (f,I) -> ideal f module I

fixup := (f) -> if isHomogeneous f then f else map(target f,,f)

Derivation Module := Module => (f,M) -> (
     R := source f;
     S := target f;
...
Derivation ** Matrix := Matrix => (f,m) -> (
     if source f =!= ring m then error "expected matrix over source ring";
     map(f ** target m, f ** source m, f cover m))

tensor(Derivation, Module) := Module => {} >> opts -> (f, M) -> f ** M
tensor(Derivation, Matrix) := Matrix => {} >> opts -> (f, m) -> f ** m

isInjective Derivation := (f) -> kernel f == 0

preimage(Derivation,Ideal) := (f,J) -> (
     R := ring J;
     kernel ( map(R/J,R) * f ))

List / Derivation := List => (v,f) -> apply(v,x -> f x)
Derivation \ List := List => (f,v) -> apply(v,x -> f x)
Derivation == ZZ := (f,n) -> (
...
ZZ == Derivation := (n,f) -> f == n

inverse Derivation := Derivation.InverseMethod = (cacheValue symbol inverse) ( f -> (
	  R := target f;
	  S := source f;
	  I := graphIdeal f;
	  G := ring I;
...
Derivation ^ ZZ := BinaryPowerMethod

map(Ring,Ring,Derivation) := Derivation => opts -> (R,S,f) -> map(R,S,matrix f,opts)

-- module maps over ring maps:
map(Module,Module,Nothing,RawMatrix) := opts -> (M,N,p,f) -> map(M,N,f)
map(Module,Module,Derivation,RawMatrix) := opts -> (M,N,p,f) -> (
     (R,S) := (ring M,ring N);
...
map(Module,Nothing,Derivation,RawMatrix) := Matrix => o -> (M,N,p,f) -> (
     d := degreeLength M;
...
map(Module,Nothing,Derivation,Matrix) := 
map(Module,Module,Derivation,Matrix) := Matrix => o -> (M,N,p,f) -> map(M,N,p,raw f,o)
map(Module,Module,Derivation,List) := Matrix => o -> (M,N,p,f) -> map(M,N,p,map(M,ring M ** N,f),o)
map(Module,Nothing,Derivation,List) := Matrix => o -> (M,N,p,f) -> map(M,N,p,map(M,,f),o)
map(Module,Derivation) := Matrix => o -> (M,p) -> map(M,,p,map(M,cover M,1),o)
